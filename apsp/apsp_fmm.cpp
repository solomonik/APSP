...
#else	
		int lda_A = (int)((&A[1][0])-(&A[0][0]));
		int lda_B = (int)((&B[1][0])-(&B[0][0]));
		int lda_C = (int)((&C[1][0])-(&C[0][0]));
		int lda_pred = (int)((&pred[1][0])-(&pred[0][0]));
//		printf("di = %d lda = %d\n", di, lda);
		fmm_naive('N', 'N', dj, di, dk,
			  (REAL*)&B[sXb][sYb], lda_B,
			  (REAL*)&A[sXa][sYa], lda_A,
			  (REAL*)&C[sXc][sYc], lda_C, 
			  (int*)&pred[sXb][sYb], (int*)&pred[sXc][sYc], lda_pred);
#endif
	}
}
